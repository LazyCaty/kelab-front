                ...
                <div className="userhead-content">
...

                    {
                        ...
                        this.state.menu.map((item,index)=>{
                            let menuLen=this.state.menu.length;
                            let spanLen=parseInt(8/menuLen);

                            return <Col key={index} span={spanLen} className="userhead-bottomItem" onMouseOver={()=>this.handleItem('itembox'+index)} onMouseLeave={()=>this.leaveItem('itembox'+index)}>
                                <div><Link to={item.key}>{item["title"]}</Link></div>
